...
We additionally wish to capture uncertainty over $w$, yielding a distribution $p(w \mid S)$.

\subsection{Architecture}

The main system used in our study has two core components: an encoder $f$ and a hypernetwork decoder $g$. We also evaluate an optional diffusion denoiser $d$ as an add-on for stochastic latent sampling.
...
\paragraph{Optional FiLM-conditioned diffusion denoiser.}
The denoiser $d(z_t, t, c)$ predicts the noise $\epsilon$ added to a noisy latent $z_t$.
It uses a sinusoidal time embedding (dimension 64) concatenated with $c$ and projected to 128 dimensions.
Four residual FiLM blocks (scale and shift computed from the conditioning vector via a linear layer) refine the latent estimate.
We use a linear variance schedule with $T=20$ diffusion timesteps ($\beta_{\min} = 10^{-4}$, $\beta_{\max} = 2 \times 10^{-2}$) and deterministic DDIM sampling at inference.
...
\paragraph{Stage 2: optional diffusion denoiser.}
The encoder is frozen.
For each step we sample a fresh batch of tasks, encode to $z$, sample a random timestep $t \sim \text{Uniform}(0, T-1)$, apply the forward diffusion $z_t = \sqrt{\bar{\alpha}_t}\, z + \sqrt{1-\bar{\alpha}_t}\, \epsilon$ with $\epsilon \sim \mathcal{N}(0, I)$, and train the denoiser to predict $\epsilon$ via MSE.
Optimiser: AdamW with learning rate $2 \times 10^{-4}$, 1000 training steps.
...
\begin{enumerate}
  ...
  \item Use the deterministic encoder latent $z$ directly for the main model, or optionally sample $K$ latent codes $\hat{z}^{(1)}, \ldots, \hat{z}^{(K)}$ via DDIM with $c$ as condition.
  \item Decode each latent code to target weights.
  \item Evaluate the resulting target networks; when diffusion sampling is used, we report both single-sample and best-of-$k$ summaries where relevant.
\end{enumerate}
The deterministic encoder path is the primary model in the paper; the diffusion path is included as an exploratory extension.

\paragraph{Static vs adaptive learners.}
The \emph{static baseline} is still a learned model, but it is trained as a single
task-agnostic predictor and does not condition on the support set at test time.
By contrast, the encoder/diffusion/direct systems are \emph{adaptive}: they ingest
support data for each episode and produce task-conditioned predictions.
Thus, reward comparisons against the static baseline measure the marginal value of
